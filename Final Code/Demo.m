...
function [result, plate] = Demo(imName)
...
%% Read Image                              
%imName = 'mulcheck.jpg';
%Im = imread(['C:\Users\protichi\Desktop\semesters\semester6\CV_project\License-Plate-Detector-master\License-Plate-Detector-master\image\' imName]);
Im = imread(imName);
...
T       = 15;                           % Extend Plate Region By T Pixel
jr      = (min(a)-T :max(a)+T);
jc      = (min(b)-T :max(b)+T);
jr      = jr(jr >= 1 & jr <= nRow);
jc      = jc(jc >= 1 & jc <= nCol);
